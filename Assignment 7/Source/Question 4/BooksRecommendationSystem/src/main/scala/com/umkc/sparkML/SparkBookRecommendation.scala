...
object SparkBookRecommendation {
  def main(args: Array[String]) {

    System.setProperty("hadoop.home.dir","C:\\Users\\npdar\\Desktop\\Acads\\BigDataAnalytics\\Assignment\\Tutorial 7\\hadoop-common-2.2.0-bin-master\\bin\\winutils")
    ...
    val books = sc.textFile(new File(movieLensHomeDir, "Books.dat").toString).map { line =>
      val fields = line.split("::")
    ...
    println("Got " + numRatings + " ratings from "
      + numUsers + " users on " + numBooks + " Books.")
    ...
    val candidates = sc.parallelize(books.keys.filter(!myRatedBookIds.contains(_)).toSeq)
    ...
    println("Books that are recommended for you:")
    recommendations.foreach { r =>
      println("%2d".format(i) + ": " + books(r.product))
      str.append("\n%2d".format(i) + ": " + books(r.product))
      i += 1
    }
